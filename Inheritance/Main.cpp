...
int main(){
    ...
    African.setFeatures("Chinua Achebe", 26
    , 1.87, 82, "male", "Nigeria");
    African.introduction();
    African.getinfo();

    // String name, int age, double height, int weight, String gender, String country
    ...
    // name, age, medicalField, employer, qualification, residency, YearsOfExperience
    Doctor Surgeon =  Doctor("female", "Alana Torres", 45, "Surgery", "Hospital", "M.D. and MCAT", "Yes", 15);
    Surgeon.getinfo();
    ...
    // Using A vector to Manipulate all created Objects and call the various instance methods 
    ...
    int count = 0;
    string tab = "\t";
    for(SoftwareEngineer item : SWE) { // Using a for each loop to modify objects in the vector 
        count++;
        cout << "\n\n" << setw(50) << tab << tab << tab << " Engineer "  << count;
        item.Bio();
        item.getinfo();
        if(count == 1 || count == 2) {
            item.Value();
    ...
    //Failed attempt at typecasting user defined types i.e classes
    // MobileDeveloper.Bio();
